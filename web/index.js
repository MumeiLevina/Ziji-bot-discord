...
async function startServer() {
	...
	server.listen(process.env.PORT || process.env.SERVER_PORT || 2003, () => {
		logger.info(`Server running on port ${process.env.PORT || process.env.SERVER_PORT || 2003}`);
	});

	if (process.env.NGROK_AUTHTOKEN && process.env.NGROK_AUTHTOKEN !== "") {
		const url = await ngrok.connect({
			addr: process.env.PORT || process.env.SERVER_PORT || 2003,
